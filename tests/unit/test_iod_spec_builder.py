...
# Captured at import time, before patch_get_table_id_from_section below ever runs, so a test that
# needs the real implementation can restore it without calling monkeypatch.undo() (which would
# also revert other patches sharing the same monkeypatch instance, e.g. conftest.py's autouse
# patch_dirs fixture).
_REAL_GET_TABLE_ID_FROM_SECTION = DOMUtils.get_table_id_from_section
...
def test_iod_spec_builder_with_module_builder_resolves_sections(monkeypatch, iod_with_module_and_section_dom):  # noqa: F811
    ...
    # implementation instead. Re-patch just that one attribute rather than calling
    # monkeypatch.undo(), which would also revert conftest.py's autouse patch_dirs fixture (it
    # shares this same monkeypatch instance) and send this test's cache/config I/O to the real,
    # unpatched platformdirs locations on disk instead of the isolated tmp path.
    monkeypatch.setattr(
        "dcmspec.iod_spec_builder.DOMUtils.get_table_id_from_section", _REAL_GET_TABLE_ID_FROM_SECTION
    )
    ...
    assert "Explanatory text for the Frame Label attribute." in section_registry["sect_C.1"].content.html