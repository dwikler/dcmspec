...
def test_parse_records_ref_columns_on_metadata(table_section_refs_dom):  # noqa: F811
    """Test that parse records the requested ref_columns on metadata, sorted."""
    parser = DOMTableSpecParser()
    column_to_attr = {0: "elem_name", 1: "elem_tag", 2: "elem_type", 3: "elem_desc"}
    metadata, _ = parser.parse(
        dom=table_section_refs_dom,
    ...
    assert metadata.ref_columns == [1, 3]


def test_parse_records_no_ref_columns_on_metadata_by_default(table_section_refs_dom):  # noqa: F811
    """Test that parse records ref_columns as None on metadata when none are requested."""
    parser = DOMTableSpecParser()
    column_to_attr = {0: "elem_name", 1: "elem_tag", 2: "elem_type", 3: "elem_desc"}
    metadata, _ = parser.parse(
        dom=table_section_refs_dom,
    ...
    assert metadata.ref_columns is None
