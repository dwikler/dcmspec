"""Section registry class for sharing section models across modules in dcmspec.

Provides the SectionRegistry class, which manages a mapping from section_id to section SpecModel.
This enables memory-efficient sharing of section models when the same section is referenced
from multiple modules.

A section_id is a string identifier for a DICOM Part 3 section, extracted from the HTML anchor
...
class SectionRegistry(UserDict):
    """Registry for sharing section models by section_id across modules.
    ...
    It is used to avoid duplicating section models in memory when the same section is
