...
- `SpecFactory.create_model` no longer relies on `input_handler.cache_file_name` being set
  for its own cache lookup, fixing a mix-up risk when reusing one factory for multiple
  calls with different `cache_file_name` values ([#138](https://github.com/dwikler/dcmspec/issues/138)).
